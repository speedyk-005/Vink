...
class Tasker:
    ...
    def __init__(self, task: Callable, once: bool = False):
        ...
        if inspect.ismethod(self.task):
            raise TypeError(
                "Bound methods are not supported due to potential reference cycles"
            )
        ...
        if len(sig.parameters) > 0:
            raise ValueError(
                f"Task function must accept 0 arguments, but has {len(sig.parameters)}"
            )
    ...
    def run(self) -> None:
        ...
        if self._once and self.lastest_result is not None:
            raise ValueError(
                "The worker has already stopped. Tasker was initialized with 'once=True'."
            )
