...
def test_search_without_filter(exact_search_strategy, sample_records):
...
def test_compact(exact_search_strategy):
    ...
    assert len(exact_search_strategy._all_ids) == ids_before - len(IDS_TO_DELETE), (
        "all_ids should be slimmed by deleted count"
    )
    assert len(exact_search_strategy._mask) == len(exact_search_strategy._all_ids), (
        "mask length should match all_ids"
    )
...
def test_save_load(sample_embeddings, tmp_path):
    ...
    assert len(strategy2._id_to_idx) == 3, (
