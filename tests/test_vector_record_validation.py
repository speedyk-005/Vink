...
@pytest.mark.parametrize("id_val, match", [
...
@pytest.mark.parametrize("callback, expected_exc, match", [
    (mock_embedding_callback, None, ""),          # Success case
    (mock_invalid_dim_callback, VectorDimensionError, "Embedding callback output dimension"),
    (mock_crashing_callback, InvalidInputError, "Embedding callback crashed"),
    (lambda x: "not-a-vector", InvalidInputError, "Vector components must be numeric"),
...
@pytest.mark.parametrize("embedding, exc_type", [
...
def test_dimension_mismatch():
    ...
    with pytest.raises(VectorDimensionError):
        VectorRecords(dim=expected_dim, records=[{"content":"test", "embedding":embedding}])
