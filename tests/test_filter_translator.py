...
@pytest.mark.parametrize(
    "filter_expr,expected_keys,expected_param",
    [
...
def test_valid_filters(translator, filter_expr, expected_keys, expected_param):
    ...
    assert all(key in clause for key in expected_keys), (
        f"Expected {expected_keys} in clause, got: {clause}"
    )
    assert params == [expected_param], (
        f"Expected params {[expected_param]}, got: {params}"
    )
...
def test_multiple_filters(translator):
    ...
    assert "category" in clause and "= ?" in clause, (
        f"Missing 'category' and '= ?' in clause: {clause}"
    )
    assert "price" in clause and ">= ?" in clause, (
        f"Missing 'price' and '>= ?' in clause: {clause}"
    )
